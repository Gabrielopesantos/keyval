...
func (s *Server) AcceptConns() {
	...
	for {
		...
		if err != nil {
			s.logger.Error("failed to establish connection: %s", err)
		}
...
func (s *Server) processConn(conn net.Conn) {
	...
	if err != nil {
		s.logger.Error("could not read command: %s", err)
		return
	}
	...
	if err != nil {
		s.logger.Error("could not get command manager for command literal '%s': %s", err, cmdLiteral)
	...
	if command.HasAdditionalArguments(cmdLiteral) {
		...
		if err != nil {
			s.logger.Error("could not parse command: %s", err)
			// Write something back
			return
		}
	}
	...
	if err != nil {
		s.logger.Error("could not write response: %s", err)
		return
	}
